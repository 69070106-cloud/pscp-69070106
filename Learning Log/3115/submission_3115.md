...
```text
รับค่า int 2 ตัวนหนึ่งบรรทัดคั่นด้วยช่องว่าง และรับค่าในลูปเวลาเปิด ปิด เป็น int สองตัว แสดงจำนวนร้านที่เปิดอยู่ในเวลาที่ต้องการเช็ค

Input:
รับค่า จำนวนร้าน และ จำนวนช่วงเวลา อยูในบรรทัดรแรก รับค่าเวลา เปิด และ ปิด ร้าน เป็น int ในลูป ตามจำนวนร้าน  และต้องอยู่ในบรรทัดเดียวกัน

Output:
พิมพ์จำนวนร้านที่เปิดนแต่ละช่วงเวลาที่ต้องการทราบโดยคั่นแต่ละช่วงด้วยช่องว่าง
```
...
```text
1. สร้าง  ALL มารับค่า จำนวนร้าน และจำนวนช่วงเวลา โดยใช้ split มาแยกตัวแปร แต่จะใช้แค่ตัวแรก สร้างตัวแปร STORE เก็บค่า ALL[0] เป็น int
2. สร้าง S_LIST ตั้งเป็นลิสว่าง ไว้เก็บช่วงเวลาเปิดปิดร้าน และ สร้าง ANS ตั้งเป็นลิสว่าง ไว้เก็บจำนวนร้านที่เปิดในแต่ละช่วงเวลา
3. สร้างลูป ตามจำนวนร้านค้า โดย ให้ TO และ TC รับเวลาเปิดปิดร้านเป็น int โดยช้คำสั่ง map และแยกตัวแปรด้วย split ใช้คำสั่ง append เก็บค่า สองตัวแปรลงใน ลิส S_LIST โดยเก็บแบบ [TO, TC]
4. ให้ NUM เก็บค่าช่วงเวลาที่ต้องการตรวจสอบ โดยใช้ split แยก
5. สร้างลูป i in NUM เพื่อเช็คแต่ละตัวที่คั่นด้วย ช่องว่าง ให้ TIME_V เก็บค่า อินเดกตัวแรกของ NUM เป็น  int และสร้าง COUNT ตั้งเป็น 0 ไว้เก็บจำนวนร้านที่เปิด ในแต่ละลูป
6. สร้างลูปในลูปเดิม โดย ใช้ for r in S_LIST และสร้างเงื่อนไข อินเดก r ต่ำแหน่ง 0 น้อยกว่าเท่ากับ TIME_V แต่น้อยกว่า อินเดก r ตำแหน่ง 1 ถ้าจริงให้บวก 1 ใน COUNT
7. ก่อนจบลูป i ให้ append COUNT ลงใน ลิส ANS
8. ปริ้น ANS โดยเติม *ไว้ข้างหน้าเพื่อลบวงเล็บและจุดคอมม่าออกให้จะเป็นตัวในลิสคั่นด้วยช่องว่าง
...
```text
เช็คนาทีสุดท้ายก่อนปิดร้านและเวลาตรงกับเวลาปิดร้าน ว่าจะออกมาเป็นยังไง
```
