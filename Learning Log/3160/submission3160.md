...
```text
รับค่าจำนวนเต็มสองตัว จากนั้นพิมจำนวนเฉพาะที่อยู่ระหว่างสองตัวแปรที่ใส่ไป

Input:
รับจำนวนเต็มสองตัว โดยต้องพิมนบรรทัดเดียวกัน

Output:
บรรทัดแรกพิมเลขจำนวนเฉาะ ที่อยู่ระหว่างสองตัวแปร
บรรทัดที่สองพิม Total primes: จำนวนของเลขเฉพาะที่ได้

Constraints:
input ต้องอยู่ในบรรทัดเดียวกัน และต้องเติม +1 ตอนสร้างลูปด้วย
```
...
```text
1 เก็บตัวแปรจำนวนเต็มสองตัวคือ START คือเลขเริ่มต้น END คือเลขตัวสุดท้าย ใช้ split เพื่อแยกตัวแปร
2 สร้างตัวแปร P_LIST =[] สร้างลิสว่างไว้เก็บค่าตัวเลขเฉพาะที่จะหาได้
3 สร้างตัวแปร PRIME = 0 ไว้เก็บจำนวนเฉพาะที่ได้
4 สร้างลูป n เริ่มที่ START,END
5 สร้างเงื่อนไขในลูป ถ้า i หาร 2,3,5 แล้วไม่มีเศษ และ i ไม่เท่ากับ 1 หรือ i เป็น 2,3,5 ให้ PRIME += 1 และเพิ่ม i เข้าไปในลิส  P_LIST
6 พิม P_LIST โดยเติม * ไว้ข้างหน้าเพื่อลบเครื่องหมาย
7 พิม Total primes: (PRIME) 
```
...
1 เก็บตัวแปรจำนวนเต็มสองตัวคือ START คือเลขเริ่มต้น END คือเลขตัวสุดท้าย ใช้ split เพื่อแยกตัวแปร
2 สร้างตัวแปร P_LIST =[] สร้างลิสว่างไว้เก็บค่าตัวเลขเฉพาะที่จะหาได้
3 สร้างลูป n (START, END+1) ต้องเพิ่ม +1 เพื่อห้ลูปนับตัวท้ายด้วย
4 สร้างเงื่อนไข ถ้า  n >1 ห้ PRIME = True
5 สร้างลูปในลูปต่อ คือลูป i (2, int(n**0.5) + 1) ใช้หลักการ คู่ตัวประกอบ จุดกึ่งกลางคือเลขนั้นรูจและคูณกัน ถ้าตัวประกอบที่น้อยกว่าหรือเท่ากับตัวรูจหารไม่ลงตัวก็ทำห้เลขนั้นเป็นจำนวนเฉพาะ
6 เงื่อนไขนลูป i ถ้า n % i คือ 0 ห้ PRIME = False และ break
7 สร้างเงื่อนไขนเงื่อนไข n > 1 ถ้า PRIME เป็นจริงให้เพิ่ม n เข้าไปในลิส P_LIST
8 สร้างเงื่อนไขหลังจากจบลูป n แล้ว ถ้า P_LIST มีค่า ให้พิม P_LIST โดยเติม *ไว้ข้างหน้าเพื่อเอาเครื่องหมายและวงเล็บออก หากP_LIST ไม่มีค่าก็ไม่ต้องพิม
9 พิม Total primes: len(P_LIST) ใช้lenเพื่อเอาแค่จำนวนข้อมูลในลิส 
```
...
```text
เช็คว่า ถ้าเจอ 0 หรือ  1 จะเป็นยังไง
```
