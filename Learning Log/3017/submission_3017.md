...
```text
โจทย์ให้เงินรวมเฉพาะแค่ค่าอาหารกับน้ำ โดยให้เขียนโค้ดคำนวณรวมทั้งค่าบริการและภาษี ว่าต้องจ่ายทั้งหมดเท่าไหร่

Input:
รับเงินรวมเฉพาะแค่ค่าอาหารกับน้ำ เป็นจำนวนเต็ม คือ NUM

Output:
พิมพ์จำนวนเงินที่ต้องรวมทุกอย่าง โดยมีจุดทศนิยม 2 ตำแหน่ง 

Constraints:
Input เป็นจำนวนเต็ม ควรพิจารณาค่าที่น้อยกว่า ห้าสิบ และเกิน หนึ่งพัน
...
```text
1).เมื่อรับค่ามาเก็บไว้ที่ตัวแปร NUM แล้ว
2).จะหาค่าบริการก่อนโดยหารร้อยแล้วคูณด้วยสิบเก็บไว้เป็นตัวแปร TA 
3).จากนั้นจะตรวจสอบเงื่อนโดยทั้งสามเงื่อนไข จะคำนวณหาภาษีโดย เอาค่าบริการบวกค่าอาหารน้ำหาร หนึ่งร้อย แล้วคูณด้วย เจ็ด เก็บค่ไว้เป็น TAX 
4).จากนั้นจะคำนวณหาเงินที่ต้องจ่ายทั้งหมดโดยเอา NUM + TA + TAX เก็บค่าไว้ที่ TOTAL  
5).ที่ต่างคือเงื่อนไขแรก หากค่าบริการต่ำกว่า ห้าสิบ จะกำหนดให้ค่าบริการเป็น ห้าสิบ ตามโจทย์ที่บอกว่าค่าบริการคิดอย่างน้อย ห้าสิบ 
6).เงื่อนไขสองหากค่าบริการมากกว่า หนึ่งพัน จะกำหนดให้ค่าบริการเป็น หนึ่งพัน ตามข้อกำหนดของโจทย์ 
7).หากนอกจากสองเงื่อนไขนี้ให้คำนวณตามปกติโดยใช้ :.2f เพื่อกำหนดทศนิยม สองตำแหน่ง
```
...
```text
เพื่อเช็คว่าหากค่าบริการต่ำกว่าห้าสิบจะเข้าเงื่อนไขแรกไหม
```
...
```text
เพื่อเช็คว่าหากค่าบริการมากกว่าหนึ่งพันจะเข้าเงื่อนไขสองไหม
```
...
```text
เพื่อเช็คว่าหากค่าบริการไม่ต่ำกว่าห้าสิบและไม่มากกว่าหนึ่งพันจะเข้าเงื่อนไขสามไหม
```
