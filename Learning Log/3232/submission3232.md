...
```text
รับค่าจำนวนเต็มสองตัวคือ ระยะกระโดดและเป้าหมาย จากนั้นพิมพ์จำนวนครั้งที่กระโดดถึงเป้าหมาย

Input:
รับจำนวนเต็มสองตัว คือ JUMP คือระยะทางที่กระโดดแต่ละครั้ง GOAL คือระยะทางที่ต้องการ

Output:
พิมพ์จำนวนครั้งที่กระโดดห้ได้ระยะทางเป้าหมาย หาก ไม่ได้พิมพ์ -1

Constraints:
input ห้อยู่นบรรทัดเดียว 
```
...
```text
1 สร้างตัวแปรไว้เก็บค่าจำนวนเต็มสองตัว คือ JUMP และ GOAL โดยช้ split แยกตัวแปรด้วยช่องว่าง
2 กำหนดให้ NOW = 0 เพื่อเก็บระยะทางที่ทำได้ TIME = 0 เพื่อเก็บจำนวนครั้งที่กระโดด
3 สร้างลูป while True
4 ให้ NOW += JUMP และ TIME += 1
5 สร้างเงื่อนไขถ้า NOW >= GOAL ให้พิมพ์ TIME และ break
6 สร้างอีกเงื่อนไขแยกกับอันก่อนถ้า NOW < GOAL และ JUMP <= 0 ให้พิมพ์ =1 และ break
7 ให้ JUMP -= 2 ก่อนจบลูปทุกครั้ง
```
...
```text
เช็คหากได้ระยะทางไม่ถึงเป้าหมาย
```
...
```text
เช็คหากกระโดดรอบเดียวถึงเป้าหมายเลย
```
