...
```text
อ่านค่าจำนวนเต็มสามค่า และพิมพ์จำนวนคนที่ได้พิจารณาของขวัญ
Input:
รับค่าจำนวนเต็มสามจำนวน คือ จำนวนคน ส่งต่อไปให้อีกคน เลขที่เป็นขโมย

Output:
พิมจำนวนคนที่ได้ตรวจสอบของขวัญ

Constraints:
input ทั้งสามตัวต้องอยู่ในบรรทัดเดียวกัน หากวนมาแล้วได้คนที่ 1 จะจบ และถ้าขโมยได้พิจารณา ก็จะจบ
```
...
```text
รับค่าจำนวนเต็มสามค่าโดยใช้splitนการแบ่งจัวแปรด้วยช่องว่างคือ N คือ จำนวนคน K คือ ส่งต่อไปให้คนถัดไปอีก K  T คือ ขโมย
ถ้า T == 1 ให้พิม 1
หากไม่ใช่จะทำตามนี้
ตั้ง COUNT = 1 เป็นตัวนับจำนวนคนที่ได้พิจารณา THINK = 1 เป็นคนปัจจุบันที่ได้พิจารณา
สร้างลูป while true 
ให้ THINK  = (THINK + K) % N ใช้ % N เพราะเมื่อเลขเกินจำนวนคนจะได้วนกลับมาใหม่ได้
ถ้า THINK == 1 ให้ break หากไม่ใช่ให้ข้ามไป
ให้ COUNT += 1
ถ้า THINK == T ให้  break
เมื่อจบลูปให้พิม COUNT
```
...
```text
ทดสอบว่าหากขโมยได้เป็นคนที่สอง
```
...
```text
ทดสอบว่าหากเกินแล้ววนกลับมาเจอโจรจะได้ไหม
```
...
```text
ทดสอบว่าถ้าวนแล้วมาเจอ 1 จะหยุดไหม
```
