...
```text
อ่านจำนวนเต็มสามตัว แล้วพิมพ์ตามที่โจทย์กำหนด

Input:
รับจำนวนเต็มสามตัว

Output:
พิมพ์ตามเงื่อนไขพิม 0 หากไม่ใช้อิฐหนึ่งเมตร พิมพ์จำนวนอิฐหนึ่งเมตรที่ใช้ ในเงื่อนไขที่ใช้อิฐเล็ก พิ่ม -1 กรณีที่อิฐไม่พอ

Constraints:
จะใช้อิฐห้าเมตรได้ก็ต่อเมื่อความยาวที่ต้องการหารห้าลงตัว หากเหลือเศษต้องใช้อิฐหนึ่งเมตรมาเสริม หากอิฐห้าเมตรพอหรือมากกว่าแต่เป้าหมายหารห้าไม่ลงตัวและอิฐหนึ่งเมตรไม่พอก็เท่ากับไม่พอ
...
```text
1. รับจำนวนอิฐหนึ่งเมตร เป็น A และอิฐห้าเมตรเป็น B และเป้าหมายที่ต้องการ เป็น GOAL เป็นจำนวนเต็มทั้งสามตัว
2 สร้างตัวแปร TOTAL_A มารับค่าความยาวทั้งหมดของอิฐหนึ่งเมตร และ TOTAL_B คือความยาวทั้งหมดของอิฐห้าเมตร โดย เอา  B มาคูณห้า
3. สร้างเงื่อนไข ถ้า TOTAL_B >= GOAL ให้พิมพ์ 0
4. สร้างเงื่อนไข ถ้า TOTAL_A + TOTAL_B >= GOAL and TOTAL_B < GOAL พิม GOAL % 5
5. หากไม่เป็นจริงทั้งสองเงื่อนไขพิมพ์ -1
```
...
```text
เพิ่มตัวแปร GOAL_B มาเก็บค่าเศษ GOAL ที่หารด้วยห้า โดย GOAL % 5 เพื่อนำไปเช็คเพิ่มในเงื่อนไข เพิ่มเงื่อนไขแรก คือ and not GOAL_B ไว้เช็คว่า GOAL ต้องมีเศษเป็นศูนย์ เปลี่ยน เงื่อนไขที่สอง เป็น 
TOTAL_B + TOTAL_A >= GOAL and TOTAL_B >= GOAL and GOAL_B and GOAL_B <= TOTAL_A เช็คว่าผลรวม มีมากกว่าหรือเท่ากับเป้าหมาย และ อิฐห้าเมตรความยาวทั้งหมดมากกว่าหรือเท่ากับเป้าหมาย และ GOAL_B ต้องไม่เป็นศูนย์ และ ความยาวอิฐหนึ่งเมตรมีมากกว่าหรือเท่ากับ GOAL_B
เพิ่มเงื่อนไขที่สาม คือ TOTAL_B + TOTAL_A >= GOAL and GOAL - TOTAL_B > 0 คือ เช็คว่าผลรวม มีมากกว่าหรือเท่ากับเป้าหมาย และ เป้าหมายลบความยาวอิฐห้าเมตรต้องมากกว่าศูนย์พิมพ์ ผลต่างของเป้าหมายลบความยาวอิฐห้าเมตร ใส่เงื่อนไขนี้เพื่อให้เงื่อนไขครอบครุมมากขึ้น 
```
...
```text
เช็คแบบเป้าหมายหารห้าลงตัวและพอ
```
...
```text
เช็คอิฐห้าเมตรไม่พอเอาอิฐเล็กมาเติม
```
