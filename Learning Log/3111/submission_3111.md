...
```text
รับค่า สตริงคือ สถามนะ และค่าจำนวนเต็ม N คือจำนวนสินค้า รับค่าตามจำนวน N  ในตัวแปรที่สอง คือราคาสินค้า แสดง ผลรวมของราคาที่ต้องจ่ายหลังจากลบส่วนลดไปแล้ว

Input:
รับค่าสตริง เป็นสถานะ เป็น STATS และรับค่าจำนวนสินค้าเป็นจำนวนเต็ม เป็น N และรับค่านลูปตามจำนวน N คือราคาสินค้าแต่ละชิ้นเป็น decimal

Output:
พิมพ์แสดงราคารวมหลังจากลบส่วนลดตามเงื่อนไข ถ้า STATS เป็น Y ได้ส่วนลด 5%  หากเป็น N แต่ยอดรวมมากกว่าหรือเท่ากับ 500 ได้ลด 3%

Constraints:
ควรอิมพอต decimal เพื่อความแม่นยำนการคำนวณทศนิยม และใช้ ROUND_HALF_UP ในการปัดเศษขึ้น
...
```text
1. อิมพอต decimal และ ใช้ , ROUND_HALF_UP เพื่อให้ปัดเศษขึ้น
2. ให้ STATS รับค่าสตริง คือ สถานะสมชิก และ N คือ รับค่า int เป็นจำนวนสินค้า
3. สร้างตัวแปร TOTAL ไว้รวมราคาทั้งหมด ตั้งเป็น 0 และเป็น decimal 
4. สร้างลูป for in range N ให้ COST รับค่าเป็น decimal และบวก COST ลงใน TOTAL
5. สร้างเงื่อไข ถ้า STATS เป็น Y ให้ TOTAL = TOTAL * 95(decimal) / 100(decimal) เพราะลด 5%
6. หาก STATS เป็น N และ TOTAL มากกว่าเท่ากับ 500 ให้ TOTAL = TOTAL * 96(decimal) / 100(decimal) เพราะลด 7%
7. ใช้คำสั่ง .quantize(Decimal('0.00'), rounding = ROUND_HALF_UP) กับ TOTAL เพื่อปัดทศนิยมห้เป็นสองต่ำแหน่งและปัดทศนิยมขึ้น เก็บเป็น TOTAL
8. ปริ้น TOTAL โดยใช้ f format :.2f เพื่อห้พิมพ์ออกมาแค่สองต่ำแหน่ง
```
...
```text
เช็คว่าเป็นสามชิกได้ส่วนลดไหม และได้ปัดทศนิยมขึ้นไหม 
```
...
```text
เช็คว่าถ้าไม่ได้เป็นสมาชิกแต่ผลรวมมากกว่า 500 จะได้ส่วนลดไหม
```
...
```text
เขาบอกว่าต้องใช้ decimal
```

คุณยังทำอะไรด้วยตนเอง

```text
โค้ดแบบยังไม่ใส่ อิมพอต decimal
```
